...
import { getStoragePublicUrl, getStoredObjectResponse, headStoredObject, resolveStorageTarget } from "../utils/storage";
...
async function handleFeed(c: AppContext, fileName: string) {
    ...
    try {
        const response = await profileAsync(c, 'rss_s3_fetch', () => getStoredObjectResponse(env, key, serverConfig));
...
async function generateFeed(
    ...
    const configReader = c?.get('serverConfig') ?? serverConfig;
    ...
    for (const [_mimeType, ext] of Object.entries(FAVICON_ALLOWED_TYPES)) {
        ...
        try {
            const response = c
                ? await profileAsync(c, 'rss_origin_favicon_fetch', () => headStoredObject(env, originFaviconKey, configReader))
                : await headStoredObject(env, originFaviconKey, configReader);
            if (response) {
    ...
    try {
        const response = c
            ? await profileAsync(c, 'rss_favicon_fetch', () => headStoredObject(env, faviconKey, configReader))
            : await headStoredObject(env, faviconKey, configReader);
        if (response) {
...
export async function rssCrontab(
    env: Env,
    db: DB,
    serverConfig?: { get: (key: string) => Promise<unknown> },
) {
    ...
    const frontendUrl = '';
    const feed = await generateFeed(env, db, frontendUrl, undefined, serverConfig);
    
    let target: Awaited<ReturnType<typeof resolveStorageTarget>>;
    try {
        target = await resolveStorageTarget(env, serverConfig);
    } catch (error) {
        console.log('[RSS] Skipping feed cache persistence:', error instanceof Error ? error.message : String(error));
        return;
    }

    if (target.type === "imgbed") {
        console.log('[RSS] Skipping feed cache persistence for imgbed storage');
        return;
    }

    // Save to object storage (if configured)
    const folder = env.S3_CACHE_FOLDER || "cache/";
    ...
    async function save(name: string, data: string) {
        ...
        try {
            if (target.type === "r2") {
                await target.bucket.put(hashkey, data, {
                    httpMetadata: {
                        contentType: name.endsWith('.json') ? 'application/json' : 'application/xml',
                    },
                });
            } else {
                const { createS3Client, putObject } = await import("../utils/s3");
                const client = createS3Client(env);
                await putObject(client, env, hashkey, data, name.endsWith('.json') ? 'application/json' : 'application/xml');
            }
